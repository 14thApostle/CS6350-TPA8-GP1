...
if dataset == 'GoPro':
    img_path = os.path.join(folder_path,r'test_gopro.png')
    model_path = os.path.join(folder_path,r'model_deblur_gopro.pt')
elif dataset == 'HIDE':
    img_path = os.path.join(folder_path,r'test_hide.png')
...
def img_to_tensor(path):
    test2 = Image.open(path)
    img_tensor = tfms(test2).unsqueeze(0)
    return img_tensor
tfms = transforms.Compose([
        transforms.ToTensor(),
       ])

a = test_model(img_to_tensor(img_path))

import imageio
#print(a)
#torch.save(a,os.path.join(folder_path,'abc.pt'))
import skimage.metrics as metrics
b = np.squeeze(a.detach().numpy()).transpose(2,1,0)
c = np.squeeze(img_to_tensor(img_path).detach().numpy()).transpose(2,1,0)
